    ...
    <table>
      <tbody className='divide-y divide-[gray]'>
        <tr className='tableRow'>
          <td className='tableDataTitle'></td>
          {products.map((prod) => (
            <td
              key={prod.id}
              className={`tableDataFeature ${selectedPlan?.id === prod.id ? 'text-[#e50914]' : 'text-[gray]'}`}
            >
              EUR{prod.prices[0].unit_amount! / 100}
            </td>
          ))}
        </tr>
